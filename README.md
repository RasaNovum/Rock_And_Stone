<h1 align="center">Rock n Stone<br>
<a href="https://www.curseforge.com/minecraft/mc-mods/rock-n-stone"><img src="https://img.shields.io/badge/CurseForge-1.20.1%20%7C%201.21.1-orange"></a>
<a href="https://modrinth.com/mod/rock-n-stone"><img src="https://img.shields.io/badge/Modrinth-1.20.1%20%7C%201.21.1-green"></a>
<a href="https://twitter.com/Rasa_Novum"><img src="https://img.shields.io/badge/Socials-Xitter-black"></a>
<a href="https://discord.gg/WGh4mq6W5U"><img src="https://img.shields.io/badge/Socials-Discord-5865F2"></a>
</h1>

Rock n Stone is a mod that changes ore distribution to follow individual sets of climate and environmental conditions, making certain ores spawn in more dedicated types of terrain than in Vanilla.

Fabric builds are provided for Minecraft 1.20.1 and 1.21.1, with a NeoForge build for Minecraft 1.21.1.

On dedicated servers, Rock n Stone is required on the server and optional on connecting clients. Install it on the client for singleplayer. Optional content mods that register blocks used by world generation, such as Surface Samples, still need to be installed wherever those blocks must exist.

Each target build also writes version-correct datapack templates to
`versions/<target>/build/templates/`.

If you have any questions that aren't [answered in our Wiki](https://github.com/RasaNovum/Rock_And_Stone/wiki), feel free to [ask on our Discord](https://discord.com/invite/WGh4mq6W5U)!

## Dependencies

- **[Fabric API](https://modrinth.com/mod/fabric-api)** - For Fabric versions
- **[NeoForge](https://neoforged.net/)** - For NeoForge version
- **[MidnightLib](https://modrinth.com/mod/midnightlib)** - Provides config

## License

**Rock n Stone** is licensed under the **Responsive Source License (RSL) v1.0**. See the [LICENSE](LICENSE.md) file for full details.

Feel free to include this mod in your modpack as long as you download it from our official sources.
